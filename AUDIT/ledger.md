...
**Open: 75 | done: 16 | blocked: 0 | total: 91** (S0 7, S1 31, S2 46, S3 7)
...
| [0033](#0033) | S1 | M1 | `chatbox.swift:1902` | SSE 'bye' frame is built with a doubled backslash, so the documented bye event is never delivered [also: SSE deadline frame uses literal backslash-n, so the bye event is never terminated] | bug | **AUDIT** | node1 | phase-B/L2-server-http,L3-line-level |
