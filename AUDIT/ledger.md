...
**Open: 89 | done: 0 | blocked: 0 | total: 89** (S0 7, S1 31, S2 44, S3 7)
...
| [0021](#0021) | S0 | M1 | `chatbox.swift:1826` | GET /ui builds 'path + ?token=' and always gets 401, so the read-only view shows an empty board [also: GET /ui concatenates window.location.search onto paths that already contain a query, so its own credential is swallowed and every fetch is unauthenticated] | bug | **START** | node1 | phase-B/L1-architecture,L2-server-http |
...
CONFIDENCE: high
- **Fix:** Require CHATBOX_URL (exit 2 naming the variable and ~/.chatbox) or default to http://127.0.0.1:8787; keep the private address only in documentation examples. | Normalise once right after config load, e.g. `while [ "${URL%/}" != "$URL" ]; do URL="${URL%/}"; done`, so every route is built with a single slash. | Default to http://127.0.0.1:8787, or refuse to run when CHATBOX_URL is unset unless the target is loopback, and keep the shell and MCP defaults identical. | Default to http://127.0.0.1:8787, or require CHATBOX_URL explicitly and exit with a clear error when it is unset.
...
- **Status:** START
- **Evidence (before):** VERIFIED BY AUDITOR (static): the loop fetches `/inbox?id=$ID&wait=$_wait` (unread only; the server caps the listing at 200 and says how many are not shown) and then acknowledges with `all=1`, so the undelivered older messages are marked read.

chatbox-cli.sh:819-820 acks `id=$ID&all=1` after printing, while the usage (80-82) and README:148-150 promise it 'acknowledges only what it delivered'. The server's ack?all=1 stamps every unread delivery for the agent (chatbox.swift:2139-2143), and GET /inbox returns only the 200 newest unread (chatbox.swift:41,799; page notice 1680-1683). A message posted between the poll response and the ack - a window as wide as the --exec command - is marked read though never shown.

WHY IT MATTERS: Durable delivery is the product's core promise ('Nothing is deleted on read', README:300; --prune never touches unread mail). This silently consumes mail: with more than 200 unread the older ones are never printed at all, and any arrival during the print/exec window is lost from the inbox for ever.

CONFIDENCE: high
- **Fix:** Ack each delivered message by id (the listing carries `[<id>] UNREAD` lines) instead of all=1, or send all=1 only when the page header reports shown == matching. Add a regression test with >200 unread and a message posted during a slow --exec.
...
CONFIDENCE: high
- **Fix:** Percent-encode '+' (and reserved characters) as %2B before assigning queryItems, e.g. port chatbox.swift's formEncode into the adapter, or POST the arguments as a properly form-encoded body. | Normalize configURL by stripping a trailing slash, or set URLComponents.path instead of concatenating, before any request is built.
...
CONFIDENCE: high
- **Fix:** Write each delivery with runReporting and check rc/changes; if any fails, delete the message and the deliveries already written and answer 500 naming the failure (or mark the answer undelivered).
...
CONFIDENCE: high
- **Fix:** Merge the query properly (path + (path.includes('?') ? '&' : '?') + query.slice(1)) or send the token as an Authorization header, and add a suite check that fetches the URLs the page builds rather than grepping the page string. | Join with the correct separator: `const url = (p) => p + (p.includes('?') ? '&' : '?') + query.replace(/^\?/, '')`, then fetch(url(path)); or build the URL with URL/URLSearchParams.
...
CONFIDENCE: high
- **Fix:** Use runReporting (or check run()'s result and sqlite3_changes) and answer 500 "the credential was not revoked" unless exactly one row was stamped; print the "rejected from now on" promise only then. | Make addToken/revokeToken return the run result (or use runReporting) and have createToken/revokeToken answer 500 'the credential was not stored/revoked' when the write failed, instead of claiming success.
...
CONFIDENCE: high
- **Fix:** Refuse to start when argPresent("--token") && tokenArg.isEmpty, exactly as the --token-file branch already does at 2826-2829 and as --backup, --prune and --tls-identity do. Only a genuinely absent --token may mean open. | Add the same fail-closed guard as the token file: `if argPresent("--token") && tokenArg.isEmpty { write to stderr; exit(1) }`, and keep token==nil meaning only 'no --token flag was given'. Pin it with a test that `--token=` refuses to start. | Refuse an empty/whitespace --token value exactly as --token-file does: if argPresent("--token") and the value trims to empty, exit 2 explaining that --token-file or the literal `open` should be used. | Make --token-file win when both are given (or refuse the combination as a mistake) and document the precedence in the usage text and README.
