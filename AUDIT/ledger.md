...
- **Fix:** Render the wake page from `/inbox?...&json=1` and acknowledge `message=<id>` for exactly the ids in that page; never derive ids from the framed text and never ack `all=1`.
- **Notes:** Largest S0 in the set; deliberately not rushed. Analysis and the rejected naive fix are recorded above.
